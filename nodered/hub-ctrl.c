...
usb_find_hubs(int listing, int verbose, int busnum, int devnum, int hub)
{
	...
	for (bus = busses; bus; bus = bus->next)
	{
		...
		for (dev = bus->devices; dev; dev = dev->next)
		{
			...
			if (listing || (verbose && ((atoi(bus->dirname) == busnum && dev->devnum == devnum) || hub == number_of_hubs_with_feature)))
				print = 1;
			...
			if (uh != NULL)
			{
				...
				if ((len = usb_control_msg(uh, USB_DIR_IN | USB_RT_HUB,
																	 ...
																	 USB_DT_HUB << 8, 0,
																	 buf, sizeof(buf), CTRL_TIMEOUT)) > sizeof(struct usb_hub_descriptor))
				{
					...
					if (print && !(uhd->wHubCharacteristics[0] & HUB_CHAR_PORTIND))
